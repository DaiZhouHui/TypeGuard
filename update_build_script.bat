...
if not exist "config\icon.ico" (
    echo 警告: 图标文件不存在，正在创建默认图标...
    python create_icon.py
    if errorlevel 1 (
        echo 错误: 无法创建默认图标
        echo 请手动将图标文件放到 config\icon.ico
    )
)
...
if not exist "config\default_config.json" (
    ...
    echo   "use_keyboard_shortcut": false, >> config\default_config.json
    ...
    echo   }, >> config\default_config.json
    echo   "keyboard_shortcut": { >> config\default_config.json
    echo     "enabled": false, >> config\default_config.json
    echo     "keys": ["F11"], >> config\default_config.json
    echo     "display": "F11" >> config\default_config.json
    echo   } >> config\default_config.json
    echo } >> config\default_config.json
    echo ✓ 已创建默认配置文件
)
...
echo   - keyboard_simulator.py (键盘模拟器)
echo   - keyboard_shortcut_test.py (快捷键测试工具)
echo.
echo 使用方法:
...
echo 2. 运行 keyboard_shortcut_test.py 测试快捷键
echo.
